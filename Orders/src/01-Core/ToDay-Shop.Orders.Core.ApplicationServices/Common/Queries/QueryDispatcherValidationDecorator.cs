...
using ToDay_Shop.Orders.Core.RequestResponse.Common;
...
public class QueryDispatcherValidationDecorator : QueryDispatcherDecorator
{
    ...
    {
        _logger.LogDebug(ApplicationConsts.QueryValidation,
                         "Validating query of type {QueryType} With value {Query}  start at :{StartDateTime}",
                         query.GetType(),
                         query,
                         DateTime.Now);
        ...
        {
            _logger.LogInformation(ApplicationConsts.QueryValidation,
