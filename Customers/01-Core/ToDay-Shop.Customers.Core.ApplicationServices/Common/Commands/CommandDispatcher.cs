...
namespace ToDay_Shop.Customers.Core.ApplicationServices.Common.Commands;
...
public class CommandDispatcher : ICommandDispatcher
{
    ...
    public async Task<CommandResult> Send<TCommand>(TCommand command) where TCommand : class, ICommand
    {
        ...
        {
            _logger.LogError(ex,
                             "There is not suitable handler for {CommandType} Routing failed at {StartDateTime}.",
                             command.GetType(),
                             DateTime.Now);
            throw;
        }
        ...
        {
            _stopwatch.Stop();
            _logger.LogInformation(ApplicationConsts.PerformanceMeasurement,
    ...
    public async Task<CommandResult<TData>> Send<TCommand, TData>(TCommand command) where TCommand : class, ICommand<TData>
    {
        ...
        {
            _stopwatch.Stop();
            _logger.LogInformation("Processing the {CommandType} command tooks {Millisecconds} Millisecconds",
                                   command.GetType(),
                                   _stopwatch.ElapsedMilliseconds);
        }
    }
